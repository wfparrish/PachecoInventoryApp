...
// @route GET api/sector3/panels
// @desc Test panels retrieval
// @access Public
  router.get('/api/sector3/panels', async (req, res) => {
    const panels = await PanelModel3.find({});
    ...
    //POST a panel
    router.post('/api/sector3/panel', async (req, res) => {
      if (req.body !== null) {
      ...
      const panel = new PanelModel3(req.body);
    ...
    //DELETE a panel
    router.delete('/api/sector3/panel/:id', async (req, res) => {
      try {
        const panel = await PanelModel3.findByIdAndDelete(req.params.id);
    ...
    //PUT a panel
    router.put('/api/sector3/panel/:id', async (req, res) => {
      try {
        await PanelModel3.findByIdAndUpdate(req.params.id, req.body);
        await PanelModel3.save();
        res.send(panel);
      } catch (err) {
        res.status(500).send(err);
      }
