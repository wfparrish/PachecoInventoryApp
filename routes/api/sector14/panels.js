...
const PanelMode14 = require('../../../models/sector14/Panel14');
const router = express.Router();

// @route GET api/sector14/panels
// @desc Test panels retrieval
// @access Public
  router.get('/api/sector14/panels', async (req, res) => {
    const panels = await PanelMode14.find({});
    ...
    //POST a panel
    router.post('/api/sector14/panel', async (req, res) => {
      if (req.body !== null) {
      ...
      const panel = new PanelMode14(req.body);
    ...
    //DELETE a panel
    router.delete('/api/sector14/panel/:id', async (req, res) => {
      try {
        const panel = await PanelMode14.findByIdAndDelete(req.params.id);
    ...
    //PUT a panel
    router.put('/api/sector14/panel/:id', async (req, res) => {
      try {
        await PanelMode14.findByIdAndUpdate(req.params.id, req.body);
        await PanelMode14.save();
        res.send(panel);
      } catch (err) {
        res.status(500).send(err);
      }
