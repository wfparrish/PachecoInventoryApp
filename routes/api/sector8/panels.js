...
// @route GET api/sector8/panels
// @desc Test panels retrieval
// @access Public
  router.get('/api/sector8/panels', async (req, res) => {
    const panels = await PanelModel8.find({});
    ...
    //POST a panel
    router.post('/api/sector8/panel', async (req, res) => {
      if (req.body !== null) {
      ...
      const panel = new PanelModel8(req.body);
    ...
    //DELETE a panel
    router.delete('/api/sector8/panel/:id', async (req, res) => {
      try {
        const panel = await PanelModel8.findByIdAndDelete(req.params.id);
    ...
    //PUT a panel
    router.put('/api/sector8/panel/:id', async (req, res) => {
      try {
        await PanelModel8.findByIdAndUpdate(req.params.id, req.body);
        await PanelModel8.save();
        res.send(panel);
      } catch (err) {
        res.status(500).send(err);
      }
