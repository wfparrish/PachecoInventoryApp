...
// @route GET api/sector5/panels
// @desc Test panels retrieval
// @access Public
  router.get('/api/sector5/panels', async (req, res) => {
    const panels = await PanelModel5.find({});
    ...
    //POST a panel
    router.post('/api/sector5/panel', async (req, res) => {
      if (req.body !== null) {
      ...
      const panel = new PanelModel5(req.body);
    ...
    //DELETE a panel
    router.delete('/api/sector5/panel/:id', async (req, res) => {
      try {
        const panel = await PanelModel5.findByIdAndDelete(req.params.id);
    ...
    //PUT a panel
    router.put('/api/sector5/panel/:id', async (req, res) => {
      try {
        await PanelModel5.findByIdAndUpdate(req.params.id, req.body);
        await PanelModel5.save();
        res.send(panel);
      } catch (err) {
        res.status(500).send(err);
      }
