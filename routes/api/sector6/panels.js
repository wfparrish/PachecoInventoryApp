...
// @route GET api/sector6/panels
// @desc Test panels retrieval
// @access Public
  router.get('/api/sector6/panels', async (req, res) => {
    const panels = await PanelModel6.find({});
    ...
    //POST a panel
    router.post('/api/sector6/panel', async (req, res) => {
      if (req.body !== null) {
      ...
      const panel = new PanelModel6(req.body);
    ...
    //DELETE a panel
    router.delete('/api/sector6/panel/:id', async (req, res) => {
      try {
        const panel = await PanelModel6.findByIdAndDelete(req.params.id);
    ...
    //PUT a panel
    router.put('/api/sector6/panel/:id', async (req, res) => {
      try {
        await PanelModel6.findByIdAndUpdate(req.params.id, req.body);
        await PanelModel6.save();
        res.send(panel);
      } catch (err) {
        res.status(500).send(err);
      }
