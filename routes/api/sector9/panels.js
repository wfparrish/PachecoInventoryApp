...
// @route GET api/sector9/panels
// @desc Test panels retrieval
// @access Public
  router.get('/api/sector9/panels', async (req, res) => {
    const panels = await PanelModel9.find({});
    ...
    //POST a panel
    router.post('/api/sector9/panel', async (req, res) => {
      if (req.body !== null) {
      ...
      const panel = new PanelModel9(req.body);
    ...
    //DELETE a panel
    router.delete('/api/sector9/panel/:id', async (req, res) => {
      try {
        const panel = await PanelModel9.findByIdAndDelete(req.params.id);
    ...
    //PUT a panel
    router.put('/api/sector9/panel/:id', async (req, res) => {
      try {
        await PanelModel9.findByIdAndUpdate(req.params.id, req.body);
        await PanelModel9.save();
        res.send(panel);
      } catch (err) {
        res.status(500).send(err);
      }
