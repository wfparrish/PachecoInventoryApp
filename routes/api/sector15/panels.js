...
const PanelMode15 = require('../../../models/sector15/Panel15');
const router = express.Router();

// @route GET api/sector15/panels
// @desc Test panels retrieval
// @access Public
  router.get('/api/sector15/panels', async (req, res) => {
    const panels = await PanelMode15.find({});
    ...
    //POST a panel
    router.post('/api/sector15/panel', async (req, res) => {
      if (req.body !== null) {
      ...
      const panel = new PanelMode15(req.body);
    ...
    //DELETE a panel
    router.delete('/api/sector15/panel/:id', async (req, res) => {
      try {
        const panel = await PanelMode15.findByIdAndDelete(req.params.id);
    ...
    //PUT a panel
    router.put('/api/sector15/panel/:id', async (req, res) => {
      try {
        await PanelMode15.findByIdAndUpdate(req.params.id, req.body);
        await PanelMode15.save();
        res.send(panel);
      } catch (err) {
        res.status(500).send(err);
      }
